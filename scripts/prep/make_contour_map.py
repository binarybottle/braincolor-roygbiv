...
Example:
python make_contour_map.py ../../data/subjects/1013_3/3/NIFTI/1013_3_glm_LabelMap.xml ../../data/subjects/1013_3/3/otl/ ../../contours/1013_3/ ./contours/1013_3/label_list.php 0 -20 0 1 512
...
NOTES regarding parameter settings in the example of NVM data:
The coronal montage is cropped and rescaled (doubled) 
to the resolution of the contours in a separate program,
so the offset in each direction needs to be calculated (upper left is zero in both)
according to the original (pre-trimmed) and cropped image dimensions and scale factor.

The contour file coronal slices are greater by:					(ydim_pretrim-stop2)
The x values in each coronal slice are R-L flopped and off by:	(-scale*start1)
The z values in each coronal slice are off by:					(-scale*(zdim_pretrim-stop3))
Where start1, stop2, and stop3 are from make_montages.py
