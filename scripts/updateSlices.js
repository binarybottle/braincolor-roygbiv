...
function update_cor(visible_cor) {
  if (visible_cor > 0) {
    var moveLeft =             visible_cor % nimages_y;
    var moveTop  = Math.floor( visible_cor / nimages_y );
...
function update_sag(visible_sag) {
  if (visible_sag > 0) {
    var moveLeft =             visible_sag % nimages_x;
    var moveTop  = Math.floor( visible_sag / nimages_x );
...
function update_hor(visible_hor) {
  if (visible_hor > 0) {
    var moveLeft =             visible_hor % nimages_z;
    var moveTop  = Math.floor( visible_hor / nimages_z );
